 ...
 *
 * Disassembly of DSDT.aml, Mon Jan 15 12:25:27 2018
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0000F9B5 (63925)
 *     Revision         0x02
 *     Checksum         0xC9
 *     OEM ID           "CST000"
 *     OEM Table ID     "CSB00000"
 *     OEM Revision     0x00000035 (53)
 *     Compiler ID      "INTL"
 *     Compiler Version 0x20120711 (538052369)
 */
DefinitionBlock ("", "DSDT", 2, "CST000", "CSB00000", 0x00000035)
{
    /*
     * iASL Warning: There were 11 external control methods found during
     * disassembly, but only 5 were resolved (6 unresolved). Additional
    ...
    External (CPST, IntObj)    // Warning: Unknown object
    ...
    Name (VGAF, One)
    OperationRegion (GNVS, SystemMemory, 0xAD9E8C18, 0x02B2)
    ...
    Scope (_SB)
    {
        ...
        Device (PCI0)
        {
            ...
            Device (LPCB)
            {
                ...
                Device (AVC1)
                {
                    Name (_HID, EisaId ("STLC537"))  // _HID: Hardware ID
                    Name (_CID, Package (0x02)  // _CID: Compatible ID
                    {
                        EisaId ("STLC537"), 
                        EisaId ("PNP0F13")
                ...
                Device (AVC3)
                {
                    Name (_HID, EisaId ("STU1B03"))  // _HID: Hardware ID
                    Name (_CID, Package (0x02)  // _CID: Compatible ID
                    {
                        EisaId ("STU1B03"), 
                        EisaId ("PNP0F13")
    ...
    Scope (_SB.PCI0.LPCB)
    {
        OperationRegion (CPSB, SystemMemory, 0xAD333E18, 0x10)
    ...
    Method (_PTS, 1, NotSerialized)  // _PTS: Prepare To Sleep
    {
        ...
        P8XH (Zero, Arg0)
        PTS (Arg0)
        Store (\_SB.PCI0.LPCB.EC0.DEVS, SHDS)
        Store (\_SB.PCI0.LPCB.EC0.DEXS, DV5A)
        Store (\_SB.PCI0.LPCB.EC0.XSLT, SLFG)
        ...
        {
            Store (\_SB.PCI0.LPCB.EC0.XSEC, \_SB.PCI0.LPCB.EC0.OSEC)
        }

        If (LEqual (Arg0, 0x04))
        {
            Store (0x04, \_SB.PCI0.LPCB.EC0.XLPT)
        }
        Else
        {
            Store (Zero, \_SB.PCI0.LPCB.EC0.XLPT)
        }
    ...
    Method (_WAK, 1, Serialized)  // _WAK: Wake
    {
        ...
        WAK (Arg0)
        ADBG ("_WAK")
        Store (SHDS, \_SB.PCI0.LPCB.EC0.DEVS)
        Store (Zero, \_SB.PCI0.LPCB.EC0.XLPT)
        If (LOr (LEqual (Arg0, 0x03), LEqual (Arg0, 0x04)))
        {
        ...
        If (LOr (LEqual (Arg0, 0x03), LEqual (Arg0, 0x04)))
        {
            ...
            If (LOr (LEqual (Arg0, 0x03), LEqual (Arg0, 0x04)))
            {
                ...
                Store (SLFG, \_SB.PCI0.LPCB.EC0.XSLT)
                Store (\_SB.PCI0.LPCB.EC0.BRGP, GP28)
            }
        ...
        If (LEqual (Arg0, 0x03))
        {
            \_SB.PCI0.SBUS.SWRB (0xE6, Zero, 0x02)
        }
    ...
    OperationRegion (SSOP, SystemIO, SSIA, 0x02)
    Field (SSOP, ByteAcc, Lock, Preserve)
    {
        TOPB,   8, 
        Offset (0x02)
    }

    Method (DWIN, 1, NotSerialized)
    {
        If (LEqual (Arg0, 0x04))
        {
            If (_OSI ("Windows 2012"))
    ...
    Scope (_SB.PCI0.LPCB)
    {
        Device (EC0)
        {
            ...
            Method (_REG, 2, NotSerialized)  // _REG: Region Availability
            {
                If (LEqual (Arg0, 0x03))
                {
                    ...
                    Else
                    {
                        ...
                        ElseIf (LOr (LEqual (OSYS, 0x07D2), LEqual (OSYS, 0x07D3)))
                        {
                            Store (0x04, OSEC)
                        }
                        ElseIf (LEqual (OSYS, 0x07D1))
                        {
                            Store (0x04, OSEC)
                        }
            ...
            Method (_Q07, 0, NotSerialized)  // _Qxx: EC Query
            {
                ...
                Sleep (0x14)
                Notify (BAT0, 0x80)
                If (LAnd (LEqual (ACWT, One), LEqual (XCIN, One)))
                {
                    Store ("D1", Debug)
                    Store (Zero, \_PR.CPU0._PPC)
                    Notify (\_PR.CPU0, 0x80)
                ...
                If (LAnd (LAnd (LEqual (HTMF, Zero), LEqual (ACWT, Zero)), LEqual (XCIN, One)))
                {
                    Store ("D4", Debug)
                ...
                If (LAnd (LEqual (HTMF, One), LEqual (ACWT, Zero)))
                {
                    If (LAnd (LAnd (LEqual (XCIN, One), LEqual (XTIN, One)), LLessEqual (BTPR, 0x28)))
                    {
                        Store ("D4", Debug)
                        Store (One, \_PR.CPU0._PPC)
                        Notify (\_PR.CPU0, 0x80)
                ...
                If (LAnd (LEqual (HTMF, One), LEqual (ACWT, Zero)))
                {
                    If (LAnd (LAnd (LEqual (XCIN, One), LEqual (XTIN, One)), LGreaterEqual (BTPR, 0x32)))
                    {
                        Store ("D1", Debug)
                        Store (Zero, \_PR.CPU0._PPC)
                        Notify (\_PR.CPU0, 0x80)
                ...
                If (LAnd (LEqual (XCIN, Zero), LEqual (XTIN, One)))
                {
                    Store ("D1", Debug)
                    Store (0x0F, \_PR.CPU0._PPC)
                    Notify (\_PR.CPU0, 0x80)
                    Store (One, GP12)
            ...
            Method (_Q2E, 0, Serialized)  // _Qxx: EC Query
            {
                Name (_T_0, Zero)  // _T_x: Emitted by ASL Compiler
                Store (0x2E, DBG8)
                Store (One, SLMS)
            ...
            Method (_Q2F, 0, Serialized)  // _Qxx: EC Query
            {
                Name (_T_0, Zero)  // _T_x: Emitted by ASL Compiler
                Store (0x2F, DBG8)
                Store (Zero, SLMS)
            ...
            Method (_Q44, 0, Serialized)  // _Qxx: EC Query
            {
                Name (_T_0, Zero)  // _T_x: Emitted by ASL Compiler
                Store (0x44, DBG8)
                Store (One, FREC)
    ...
    Scope (_SB)
    {
        Device (BAT0)
        {
            ...
            {
                Store ("BIF1", Debug)
                Store (^^PCI0.LPCB.EC0.XIF0, Index (PAK0, Zero))
                Store (^^PCI0.LPCB.EC0.XIF1, Index (PAK0, One))
                Store (^^PCI0.LPCB.EC0.XIF2, Local0)
    ...
    Scope (\)
    {
        OperationRegion (PMS0, SystemIO, PMBS, 0x40)
        ...
        OperationRegion (SHEC, SystemIO, 0x66, One)
        Field (SHEC, ByteAcc, NoLock, Preserve)
        {
            SECM,   8
        }

        OperationRegion (SKLG, SystemIO, 0x62, One)
        Field (SKLG, ByteAcc, NoLock, Preserve)
        {
            SKLD,   8
        }
    }
    ...
    Scope (_SB)
    {
        Device (AMW0)
        {
            ...
            Method (OEMF, 1, NotSerialized)
            {
                ...
                {
                    If (LOr (LEqual (APFS, One), LEqual (RSCN, One)))
                    {
                        Store (One, SAC0)
                        Store (Zero, APFS)
                        Store (Zero, RSCN)
                    }
                    ElseIf (LAnd (LEqual (FLSH, Zero), LEqual (RSCN, Zero)))
                    {
                        Store (Zero, SAC0)
                    }
                    Else
                ...
                If (LEqual (SAC1, 0x2800))
                {
                    If (LEqual (GP28, One))
                    {
                        Store (0x64, SAC0)
                    }
                    ElseIf (LEqual (GP28, Zero))
                    {
                        Store (0x07, SAC0)
                    }
                    Else
    ...
    OperationRegion (HSUS, SystemIO, 0xF040, 0x17)
    Field (HSUS, ByteAcc, Lock, Preserve)
    {
        HSTS,   8, 
        Offset (0x02), 
        HSTC,   8, 
    ...
    Method (AHST, 1, NotSerialized)
    {
        If (LEqual (Arg0, 0x03))
        {
            \_SB.PCI0.SBUS.SWRB (0xE6, Zero, 0x04)
            Store (0x7B, IND1)
            Store (\_SB.PCI0.SBUS.SRDB (0xE6, Zero), DAT1)
            Sleep (0x03E8)
        }
    ...
    OperationRegion (OGNS, SystemMemory, 0xAD9E9E18, 0x0005)
